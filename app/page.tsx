  ...
  return (
    <div id="top">
      ...
      {checkoutOpen && <div className="checkout-backdrop"><section className="checkout-modal" role="dialog" aria-modal="true" aria-label="Finalizar pedido">
        ...
        </div><aside className="order-review"><span>RESUMO</span>{cartItems.map(({ product, quantity }) => <div className="review-item" key={product.id}><small>{product.name}<em>{money(product.price)} cada</em></small><div><button onClick={() => removeOne(product.id)} aria-label={`Diminuir ${product.name}`}>−</button><b>{quantity}</b><button onClick={() => setCart(current => [...current, product])} aria-label={`Aumentar ${product.name}`}>+</button></div><strong>{money(product.price * quantity)}</strong></div>)}<p><small>Produtos</small><b>{money(cartTotal)}</b></p>{customer.fulfillment === "entrega" && <p><small>Entrega{selectedNeighborhood ? ` · ${selectedNeighborhood.name}` : ""}</small><b>{selectedNeighborhood ? money(deliveryFee) : "Escolha o bairro"}</b></p>}<div><span>Total</span><strong>{money(orderTotal)}</strong></div>{orderError && <p className="order-error" role="alert">{orderError}</p>}<button onClick={submitOrder} disabled={submittingOrder || !cartItems.length || !customer.name.trim() || !customer.phone.trim() || customer.fulfillment === "entrega" && (!customer.address.trim() || !customer.neighborhoodId)}>{submittingOrder ? "Registrando..." : "Confirmar pedido"}</button><small className="pending-whatsapp">Você pode ajustar as quantidades aqui antes de confirmar. O pedido ficará salvo no painel e o resumo será copiado para o WhatsApp.</small></aside></div>
        </>}
      </section></div>}
