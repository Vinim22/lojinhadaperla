...
export default function AdminPanel({ userName, signOutHref }: { userName: string; signOutHref: string }) {
  ...
  const [orderSort, setOrderSort] = useState<"recent" | "oldest" | "highest" | "lowest">("recent");
  ...
  const hasProductFilters = Boolean(query.trim()) || status !== "all" || categoryFilter !== "all" || highlightFilter !== "all";
  ...
  const filteredOrders = useMemo(() => orders.filter(order => {
    const orderProducts = orderItems.filter(item => item.orderId === order.id).map(item => item.productName).join(" ");
    const matchesText = `${order.id} ${order.customerName} ${order.customerPhone} ${order.neighborhood || ""} ${orderProducts}`.toLowerCase().includes(orderQuery.toLowerCase());
    const matchesStatus = orderStatusFilter === "all" || order.status === orderStatusFilter;
    const matchesFulfillment = fulfillmentFilter === "all" || order.fulfillment === fulfillmentFilter;
    return matchesText && matchesStatus && matchesFulfillment;
  }).sort((a, b) => {
    if (orderSort === "highest") return b.total - a.total;
    if (orderSort === "lowest") return a.total - b.total;
    const aTime = new Date(a.createdAt.replace(" ", "T") + "Z").getTime();
    const bTime = new Date(b.createdAt.replace(" ", "T") + "Z").getTime();
    return orderSort === "oldest" ? aTime - bTime : bTime - aTime;
  }), [orders, orderItems, orderQuery, orderStatusFilter, fulfillmentFilter, orderSort]);
  const openOrders = orders.filter(order => !["concluido", "cancelado"].includes(order.status));
  const todayOrders = orders.filter(order => {
    const created = new Date(order.createdAt.replace(" ", "T") + "Z");
    return created.toLocaleDateString("pt-BR") === new Date().toLocaleDateString("pt-BR");
  });
  ...
  const toggleVisibleSelection = () => {
    setSelectedProductIds(current => allVisibleSelected ? current.filter(id => !visibleProductIds.includes(id)) : [...new Set([...current, ...visibleProductIds])]);
  };
  const bulkAvailability = async (nextAction: "activate" | "deactivate") => {
  ...
  return <div className="admin-app">
    ...
    <div className="admin-workspace">
        ...
        <section className="admin-product-panel">
          ...
          {loading ? <p className="admin-empty">Carregando catálogo...</p> : filteredProducts.length ? <div className="admin-product-table">
            ...
            {filteredProducts.map(product => <article key={product.id} className={!product.available ? "unavailable" : ""}>
              ...
              <div className="admin-category-tags">{product.category.split(",").map(category => category.trim()).filter(Boolean).map(category => <span key={category}>{category}</span>)}</div><strong>{money(product.price)}</strong><span className={`status-pill ${product.available ? "on" : "off"}`}>{product.available ? "Ativo" : "Esgotado"}</span>
      ...
      {section === "orders" && <main className="admin-page admin-simple-page"><div className="admin-page-heading"><div><small>GERENCIAMENTO</small><h1>Pedidos</h1><p>Acompanhe, filtre e atualize os pedidos recebidos pela loja.</p></div><button className="admin-cta" onClick={loadOrders}>Atualizar</button></div><section className="order-dashboard"><article><span>Abertos</span><strong>{openOrders.length}</strong></article><article><span>Hoje</span><strong>{todayOrders.length}</strong></article><article><span>Total filtrado</span><strong>{money(filteredOrders.reduce((sum, order) => sum + order.total, 0))}</strong></article><article><span>Ticket médio</span><strong>{money(filteredOrders.length ? filteredOrders.reduce((sum, order) => sum + order.total, 0) / filteredOrders.length : 0)}</strong></article></section><section className="order-filters"><label><span>⌕</span><input value={orderQuery} onChange={event => setOrderQuery(event.target.value)} placeholder="Buscar cliente, telefone, produto ou pedido" /></label><select value={orderStatusFilter} onChange={event => setOrderStatusFilter(event.target.value)}><option value="all">Todos os status</option><option value="novo">Novo</option><option value="confirmado">Confirmado</option><option value="preparando">Preparando</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select><select value={fulfillmentFilter} onChange={event => setFulfillmentFilter(event.target.value)}><option value="all">Entrega e retirada</option><option value="entrega">Entrega</option><option value="retirada">Retirada</option></select><select value={orderSort} onChange={event => setOrderSort(event.target.value as typeof orderSort)}><option value="recent">Mais recentes</option><option value="oldest">Mais antigos</option><option value="highest">Maior valor</option><option value="lowest">Menor valor</option></select></section>{ordersLoading ? <p className="admin-empty">Carregando pedidos...</p> : orders.length ? filteredOrders.length ? <section className="orders-grid">{filteredOrders.map(order => <article key={order.id} className={`order-admin-card order-status-${order.status}`}><header><div><small>PEDIDO</small><h2>#{order.id}</h2></div><select value={order.status} onChange={event => updateOrderStatus(order.id, event.target.value)}><option value="novo">Novo</option><option value="confirmado">Confirmado</option><option value="preparando">Preparando</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select></header><div className="order-customer"><strong>{order.customerName}</strong><a href={`tel:${order.customerPhone}`}>{order.customerPhone}</a><small>{order.fulfillment === "entrega" ? `Entrega${order.neighborhood ? ` · ${order.neighborhood}` : ""}` : "Retirada na loja"} · {new Date(order.createdAt.replace(" ", "T") + "Z").toLocaleString("pt-BR")}</small><div className="order-contact-actions"><a href={`tel:${order.customerPhone}`}>Ligar</a><a href={orderWhatsAppLink(order)} target="_blank" rel="noreferrer">WhatsApp</a></div></div><div className="order-admin-items">{orderItems.filter(item => item.orderId === order.id).map((item, index) => <p key={`${order.id}-${index}`}><span>{item.quantity}x {item.productName}</span><b>{money(item.unitPrice * item.quantity)}</b></p>)}</div><div className="order-totals"><p><span>Produtos</span><b>{money(order.subtotal)}</b></p>{order.deliveryFee > 0 && <p><span>Entrega</span><b>{money(order.deliveryFee)}</b></p>}</div><footer><span>Total</span><strong>{money(order.total)}</strong></footer></article>)}</section> : <section className="coming-card"><i>⌕</i><h2>Nenhum pedido no filtro</h2><p>Ajuste a busca ou selecione outro status.</p></section> : <section className="coming-card"><i>▣</i><h2>Nenhum pedido registrado</h2><p>Os novos pedidos feitos na loja aparecerão aqui.</p></section>}</main>}
    </div>
  </div>;
}
