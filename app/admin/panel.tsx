...
type Product = {
  id: number; name: string; description: string; price: number; oldPrice: number | null;
...
export default function AdminPanel({ userName, signOutHref }: { userName: string; signOutHref: string }) {
  ...
  useEffect(() => {
    // The initial request hydrates this client-only management surface.
  ...
  const submit = async (event: FormEvent) => {
    ...
    setMessage(editing ? "Produto atualizado com sucesso." : "Produto cadastrado com sucesso."); reset(); await load(); setSection("catalog");
  };
